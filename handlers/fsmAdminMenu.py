    ...
    if not message.from_user.id in ADMINS:
        ...
        if message.chat.type == "private":
            ...
            await message.answer(
                f"Хало {message.from_user.first_name} " f"покажи пиццу.",
...
async def load_pizza_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await message.reply(
            f"что такое {data['pizza_name']}?", reply_markup=cancel_markup
        )
    await FSMAdmin.next()
            ...
            await bot.send_photo(
                ...
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton(
                        f"delete {dish[1]}", callback_data=f"delete {dish[1]}"
                    )
                ),
            )
...
def register_handlers_fsm_admin(dp: Dispatcher):
    dp.register_message_handler(
        cancel_registration, state="*", commands=["cancel"], commands_prefix="\!."
    )
    dp.register_message_handler(
        cancel_registration, Text(equals="cancel", ignore_case=True), state="*"
    )
    dp.register_message_handler(fsm_start, commands=["create"])
    dp.register_message_handler(
        load_photo, state=FSMAdmin.pizza_photo, content_types=["photo"]
    )
